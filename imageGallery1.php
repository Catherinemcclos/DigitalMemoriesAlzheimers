...
<!-- delete script -->
    ...
    {
        $stmt_select=$db_conn->prepare('SELECT * FROM Users WHERE id=:userID');
        $stmt_select->execute(array(':userID'=>$_GET['delete_id']));
        $imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
        unlink("uploads/".$imgRow['picProfile']);
        $stmt_delete=$db_conn->prepare('DELETE FROM Users WHERE id =:userID');
        $stmt_delete->bindParam(':userID', $_GET['delete_id']);
        if($stmt_delete->execute())
        {
            ?>
            <script>
            alert("You are deleted one item");
            window.location.href=('imageGallery1.php');
            </script>
            <?php 
        }else
...
<?php 
    ...
    if(isset($_POST['btn-add']))
    {
        ...
        $stmt=$db_conn->prepare('INSERT INTO Users(Username, picProfile) VALUES (:uname, :upic)');
